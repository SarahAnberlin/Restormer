...
def save_files(file_):
    ...
    lq_filename = os.path.splitext(os.path.split(lr_file)[-1])[0]
    hq_filename = os.path.splitext(os.path.split(hr_file)[-1])[0]
    ...
    if w > p_max and h > p_max:
        ...
        for i in w1:
            for j in h1:
                ...
                lr_savename = os.path.join(lr_tar, lq_filename + '-' + str(num_patch) + '.png')
                hr_savename = os.path.join(hr_tar, hq_filename + '-' + str(num_patch) + '.png')

                cv2.imwrite(lr_savename, lr_patch)
                cv2.imwrite(hr_savename, hr_patch)

    else:
        lr_savename = os.path.join(lr_tar, lq_filename + '.png')
        hr_savename = os.path.join(hr_tar, hq_filename + '.png')
